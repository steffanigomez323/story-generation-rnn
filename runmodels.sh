...
datasets=( "nightvale" "quentintarantino" "smallshakespeare" "xfilesalloriginal" )
...
do
    ...
    python run.py $d lda data/$d.txt $out -doclen 50
   
    python run.py $d lda data/$d.txt $out -num_topics $numtopics -doclen $doclen
    python run.py $d lda data/$d.txt $out -num_topics $numtopics -doclen $doclen -num_epochs $epoch
